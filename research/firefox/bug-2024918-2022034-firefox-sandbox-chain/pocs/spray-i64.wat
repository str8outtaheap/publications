(module
  ;; Pseudocode: type Spray = { field0: i64 }
  (type $Spray (struct (field i64)))

  ;; Pseudocode: const table = [null]
  (table $table 1 anyref)

  (func (export "spray") (param $count i32) (param $value i64)
    (local $i i32)
    (local $cell (ref null $Spray))
    ;; Pseudocode: do {
    (loop $loop
      ;; Pseudocode: cell = new Spray(value)
      (local.set $cell (struct.new $Spray (local.get $value)))
